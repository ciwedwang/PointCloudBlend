...
void on_get_hdl_cloud_for_loop(const CloudIConstPtr& cloud){
    ...
    PointCloudHelper::cloudi2cloud(cloud, cloud_temp, 11, false, 40,
        get_glass_min_ins(cloud->header.stamp / 1000, g_status._glass_stamp));
    //PointCloudHelper::cloudi2cloud(cloud, cloud_temp, 11, false, 40, 10);
...
void process_wall(string pcap_file, bool isBatch){
    ...
    if (isBatch) {
        ...
        //        continue;
        //    }
        //    FileHelper::write_file_rot_z(i, g_status._pcap_file,
        //        plane._mat_rot * g_status._optimize_rot[i], i == 0 ? false : true,
        //        g_status._clouds_line[i]->header.stamp / 1000);
        //}
	} else {
		while (true) {
...
void test_icp_bug()
{
	string pcd1 = "E:\\PointCloud\\PointIcpReg0723\\test_0.pcd";
	string pcd2 = "E:\\PointCloud\\PointIcpReg0723\\test_0.pcd";
    CloudPtr cloud1(new Cloud);
    CloudPtr cloud2(new Cloud);
    pcl::io::loadPCDFile(pcd1, *cloud1);
    pcl::io::loadPCDFile(pcd2, *cloud2);
	if (cloud1->size() > 3000000)
	{
		PointCloudHelper::remove_duplicate(cloud1, 0.3);
	}
	if (cloud2->size() > 3000000)
	{
		PointCloudHelper::remove_duplicate(cloud2, 0.3);
	}
	Matrix4f icp_trans_mat = Matrix4f::Identity();
	float dis = PointCloudHelper::get_rot_icp(cloud1, cloud2, icp_trans_mat, true);
}
...
int main(int argc, char** argv)
{
    ...
    g_grabber = new pcl::HDLGrabber(string(""), g_status._pcap_file,g_status._current_dev);
    g_grabber->setFrameCountIgnore(g_status._frame_count_total - 1);
    CloudPtr cloud_line_result(new Cloud);
    ...
    if (g_status._current_mode == ENUM_MODE_PROCESS_POS) {
        ...
        delete g_grabber;
        g_grabber = NULL; 
        g_grabber = new pcl::HDLGrabber(string(""), g_status._pcap_file,g_status._current_dev, &imuFrame);
        g_grabber->setFrameCountIgnore(g_status._frame_count_total - 1);
...
int main(int argc, char** argv){
    ...
    if (p_time->tm_year + 1900 > 2017)// || p_time->tm_mon + 1 >= 8)
    {
        return 0;
    }
