...
int gpc_solve(int K, const std::vector< gpc_corr>& c, const double* x0, const double* cov_x0, double* x_out){
    (void) x0; (void) cov_x0;
    ...
    double p[3] = {
        m_dot(m2t, m2) - 2 * m_dot(m2t, m3) + m_dot(m3t, m3), 4 * m_dot(m2t, m1) - 8 * m_dot(m2t, g2) + 4 * m_dot(g2t, m3), 4 * m_dot(m1t, m1) - 8 * m_dot(m1t, g2) + 4 * m_dot(g2t, g2)
    };

    double l[3] = {
        m_det(mS), 2 * gmg(mS, 0, 0) + 2 * gmg(mS, 1, 1), 4
    };

    /* q = p - l^2        */
    double q[5] = {
        p[0] - (l[0] * l[0]), p[1] - (2 * l[1] * l[0]), p[2] - (l[1] * l[1] + 2 * l[0] * l[2]), -(2 * l[2] * l[1]), -(l[2] * l[2])
    };
